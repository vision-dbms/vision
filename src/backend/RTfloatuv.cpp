/*****  Float U-Vector Representation Type Handler  *****/

/***************************
 *****  Facility Name  *****
 ***************************/

#define FacilityName rtFLOATUV
...
DEFINE_CONCRETE_RTT (rtFLOATUV_Handle);

...
/*---------------------------------------------------------------------------
 *****  Basic FloatUV instantiation routine.
 *
 *  Arguments:
 *	pPPT			- a handle for the PPT of the result.
 *	pRPT			- a handle for the RPT of the result.
 ...
 *
 *****/
PublicFnDef M_CPD * __cdecl rtFLOATUV_New (
...
/*****  Create and initialize the FloatUV  *****/
    return UV_New (
	RTYPE_C_FloatUV, pPPT, pRPT, sizeof (rtFLOATUV_ElementType), initFn, initFnAP
    );
}

...
/*---------------------------------------------------------------------------
 *****  Standard FloatUV instantiation routine.
 *
 *  Arguments:
 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_New (rtPTOKEN_Handle *pPPT, M_CPD *pRPTRef, int xRPTRef) {
    return UV_New (
	RTYPE_C_FloatUV,
	pPPT,
	pRPTRef,
	xRPTRef,
	sizeof (rtFLOATUV_ElementType),
 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_New (rtPTOKEN_Handle *pPPT, rtPTOKEN_Handle *pRPT) {
    return UV_New (
	RTYPE_C_FloatUV, pPPT, pRPT, sizeof (rtFLOATUV_ElementType), NilOf (Ref_UV_Initializer), 0
    );
}

...
/*---------------------------------------------------------------------------
 *****  Routine to positionally align a FloatUV.
 *
 *  Argument:
 *	cpd			- a CPD for the FloatUV to be aligned.
 *
 *  Returns:
 *	'cpd'
 *
 *****/
PrivateFnDef M_CPD *rtFLOATUV_Align (M_CPD *cpd) {
/*****  Validate Argument R-Type  *****/
    RTYPE_MustBeA ("rtFLOATUV_Align", cpd->RType (), RTYPE_C_FloatUV);

/*****  Align Positionally  *****/
    static_cast<rtFLOATUV_Handle*>(cpd->containerHandle ())->align ();
...
/*****  If the source is empty, ... *****/
	...
	*resultCPD = rtFLOATUV_New (pResultPPT, sourceCPD, UV_CPx_RefPToken);
    }
/*****  ... otherwise, ...  *****/
    else {
    /*****  ... sort the source, ... *****/
	rtFLOATUV_ElementType const *const pSourceArray = rtFLOATUV_CPD_Array (sourceCPD);
	...
	unsigned int const *const pSortLimit = pSortArray + sSource;
        ...
        UTIL_FloatQSort (pSourceArray, pSortArray, sSource, false);
	...
	*resultCPD = rtFLOATUV_New (pResultPPT, sourceCPD, UV_CPx_RefPToken);
	rtFLOATUV_ElementType *const pResultArray = rtFLOATUV_CPD_Array (*resultCPD);
...
/*---------------------------------------------------------------------------
 *****  Internal routine to initialize a 'DoubleUV' routine from an 'FloatUV'.
 *
 *  Arguments:
 ...
 *
 *****/
PrivateFnDef void InitializeDoubleUVFromFloatUV (
	...
	*sp = rtFLOATUV_CPD_Array (iArgList.arg<M_CPD*>()),
	*sl = sp + nelements;
...
/*---------------------------------------------------------------------------
 *****  Routine to convert a FloatUV to a DoubleUV
 *
 *  Arguments:
 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_AsDoubleUV (M_KOT *pKOT, M_CPD *sourceCPD) {
    rtFLOATUV_Align (sourceCPD);
	...
	pSourcePPT, pKOT->TheDoublePTokenHandle (), InitializeDoubleUVFromFloatUV, sourceCPD
    );
}

...
/*---------------------------------------------------------------------------
 *****  Internal routine to initialize a 'IntUV' routine from a 'FloatUV'.
 *
 *  Arguments:
 *	resultCPD		- a initialized standard CPD for the u-vector
 *				  being created.
 ...
 *
 *****/
PrivateFnDef void InitializeIntUVFromFloatUV (
    M_CPD *resultCPD, size_t nelements, va_list ap
) {
    V::VArgList iArgList (ap);
    rtINTUV_ElementType		*tp = rtINTUV_CPD_Array (resultCPD);
    rtFLOATUV_ElementType	*sp = rtFLOATUV_CPD_Array (iArgList.arg<M_CPD*>()),
				*sl = sp + nelements;
...
/*---------------------------------------------------------------------------
 *****  Routine to convert a FloatUV to a IntUV
 *
 *  Arguments:
 *	pKOT			- the known object table of the result.
 *	sourceCPD		- a standard CPD for the u-vector to be
 *				  converted.
 *	ppGuard			- the return address for the domain of definition
 *				  injection which will be created if this routine
 *				  detects values not convertable to integers.  Set
 *				  to Nil if the mapping is total.
 *
 *  Returns:
 *	A standard CPD for the IntUV created.
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_AsIntUV (M_KOT *pKOT, M_CPD *sourceCPD, rtLINK_CType **ppGuard) {
    rtFLOATUV_Align (sourceCPD);
    ...
    return rtINTUV_New (
	pPPT, pKOT->TheIntegerPTokenHandle (), InitializeIntUVFromFloatUV, sourceCPD, pPPT.referent (), ppGuard
    );
}

PublicFnDef M_CPD *rtFLOATUV_AsIntUV (M_KOT *pKOT, M_CPD *sourceCPD) {
    rtLINK_CType *pDDLC;
    M_CPD *resultUV = rtFLOATUV_AsIntUV (pKOT, sourceCPD, &pDDLC);
    pDDLC->release ();
    return resultUV;
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_Distribute (M_CPD *refuvCPD, M_CPD *sourceuvCPD) {
    rtPTOKEN_Handle::Reference pPPT (
    ...
    return rtFLOATUV_UVAssign (rtFLOATUV_New (pPPT, pRPT), refuvCPD, sourceuvCPD);
}

 ...
 *
 *  Arguments:
 *	sourceCPD		- a standard CPD for the FloatUV from which
 ...
 *
 *  Returns:
 *	A standard CPD for the FloatUV containing the extracted elements.
 *
 *  Notes:
 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_LCExtract (M_CPD *sourceCPD, rtLINK_CType *linkConstructor) {
/*****
 *  Align the link constructor and validate its applicability as an extraction
 *  subscript for 'source'
 *****/
    linkConstructor->AlignForExtract (rtFLOATUV_Align (sourceCPD), UV_CPx_PToken);
...
/*---------------------------------------------------------------------------
 *****  Internal 'rtFLOATUV_New' initialization routine to extract a
 *****  collection of elements from a FloatUV.
 *
 *  Arguments:
 *	extractedUVCPD		- an initialized standard CPD for the
 *				  FloatUV being extracted.
 *	nelements		- the number of elements in the FloatUV
 *				  being extracted.
...
PrivateFnDef void InitializeExtractedUV (
	...
	*ip, *il, index,
    ...
    for (source = rtFLOATUV_CPD_Array (sourceCPD),
	 tp = rtFLOATUV_CPD_Array (extractedUVCPD),
	 il =
	     UV_CPD_ElementCount (refuvCPD) +
	         (ip = rtREFUV_CPD_Array (refuvCPD));
	 ip < il;
	 *tp++ = (index = *ip++) >= iRefNil ? (float)0.0 : source[index]);
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_UVExtract (M_CPD *sourceCPD, M_CPD *refuvCPD) {
/*****
 * Align 'source' and 'refuv' and validate 'refuv's applicability as an
 * extraction subscript for 'source'.
 *****/
    rtREFUV_AlignForExtract (rtFLOATUV_Align (sourceCPD), UV_CPx_PToken, refuvCPD);
...
/*****  Extract and return the requested values  *****/
    ...
    rtPTOKEN_Handle::Reference pRPT (static_cast<rtUVECTOR_Handle*>(sourceCPD->containerHandle ())->rptHandle ());
    return rtFLOATUV_New (pPPT, pRPT, InitializeExtractedUV, sourceCPD, refuvCPD);
}

 ...
 *
 *****/
PublicFnDef void rtFLOATUV_RFExtract (
 ...
 *****/
	...
	referenceAddr, rtFLOATUV_Align (sourceCPD), UV_CPx_PToken
    );

/*****  Extract and return the requested values  *****/
    element = rtREFUV_Ref_Element (referenceAddr);
    *resultAddr = element >= referenceAddr->RPTCardinality ()
	? (rtFLOATUV_ElementType) 0.0
	: rtFLOATUV_CPD_Array (sourceCPD)[element];
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_UVAssign (
    M_CPD *targetCPD, M_CPD *refuvCPD, M_CPD *sourceCPD
)
{
    rtFLOATUV_ElementType	*target, *sp;
    int				*ip, *il;
...
/*****  Do the assignment and return  *****/
    ...
    for (target = rtFLOATUV_CPD_Array (targetCPD),
	 sp = rtFLOATUV_CPD_Array (sourceCPD),
	 il =
	     UV_CPD_ElementCount (refuvCPD) +
	         (ip = rtREFUV_CPD_Array (refuvCPD));
	 ip < il;
 ...
 *
 *****/
PublicFnDef void rtFLOATUV_RFAssign (
    M_CPD			*targetCPD,
 ...
 *****/
    rtREFUV_AlignAndValidateRef (
	reference, rtFLOATUV_Align (targetCPD), UV_CPx_PToken
    );

/*****  Do the assignment and return  *****/
    targetCPD->EnableModifications ();
    UV_CPD_IsASetUV (targetCPD) = false;
    rtFLOATUV_CPD_Array (targetCPD)
 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_LCAssignScalar (
    M_CPD *targetCPD, rtLINK_CType *linkConstructor, rtFLOATUV_ElementType *valuePtr
) {
 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_UVAssignScalar (
    M_CPD *targetCPD, M_CPD *refuvCPD, rtFLOATUV_ElementType *valuePtr
)
{
    rtFLOATUV_ElementType	*target, value = *valuePtr;
    int				*ip, *il;
 ...
 *****/
	...
	(rtFLOATUV_Align (targetCPD), UV_CPx_PToken, refuvCPD);
 ...
 *
 *****/
PublicFnDef M_CPD *rtFLOATUV_PartitndSortIndices (
...
/*****  Align and validate parameters...  *****/
    partition->AlignForDistribute (rtFLOATUV_Align (values), UV_CPx_PToken);

/*****  Create the sort indices...  *****/
    M_CPD *result = rtREFUV_New (partition->PPT (), partition->PPT ());

/*****  Initialize the traversal pointers...  *****/
    rtFLOATUV_ElementType const *valuesArray = rtFLOATUV_CPD_Array (values);
 ...
 *
 *****/
PublicFnDef void rtFLOATUV_PartitndPartition (
 ...
 *---------------------------------------------------------------------------*/
    ...
    rtFLOATUV_ElementType previousValue, currentValue;
...
/*****  Initialize the traversal pointers...  *****/
    rtFLOATUV_ElementType const *valuesPtr = rtFLOATUV_CPD_Array (values);
...
/*---------------------------------------------------------------------------
 *****  Routine to locate or add a set of values to a float set uvector.
 *
 *  Arguments:
 *	targetCPD	- a standard CPD for a float set uvector for the
 *                        uvector to be added to.
 *	sourceCPD	- a standard CPD for a float set uvector containing
 ...
 *
 *****/
PublicFnDef void rtFLOATUV_LocateOrAdd (
    M_CPD*			targetCPD,
    M_CPD*			sourceCPD,
...
/***** Make sure both the source and target are float set uvectors ... *****/
    if ((sourceCPD->RType () != RTYPE_C_FloatUV) ||
	!UV_CPD_IsASetUV (sourceCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtFLOATUV_LocateOrAdd:  Source UVector Must Be A Float Set"
    );

    if ((targetCPD->RType () != RTYPE_C_FloatUV) ||
	!UV_CPD_IsASetUV (targetCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtFLOATUV_LocateOrAdd:  Target UVector Must Be A Float Set"
    );
...
/***** Align and validate source and target uvectors ... *****/
	...
	EC__InternalInconsistency,
	"rtFLOATUV_LocateOrAdd:  Referential Inconsistency"
    );
...
/***** Allocate space for the new target values ... *****/
    ...
    resultStart = resultp = (rtFLOATUV_ElementType *) UTIL_Malloc (
	largestSize * sizeof (rtFLOATUV_ElementType)
    );


/***** Begin the traversal ... *****/
    resultCount = 0;
    srcp = rtFLOATUV_CPD_Array (sourceCPD);
    srcpl = srcp + UV_CPD_ElementCount (sourceCPD);
    srcPos = 0;
    trgp = rtFLOATUV_CPD_Array (targetCPD);
...
/***** If values were added ... *****/
	...
	rtFLOATUV_CPD_Element (targetCPD) =
	    rtFLOATUV_CPD_Array (targetCPD) + origTargetCount;
...
/*---------------------------------------------------------------------------
 *****  Routine to locate or add a value to a float set uvector.
 *
 *  Arguments:
 *	targetCPD	- a standard CPD for a float set uvector for the
 *                        uvector to be added to.
 ...
 *
 *****/
PublicFnDef bool rtFLOATUV_ScalarLocateOrAdd (
    M_CPD *targetCPD, rtFLOATUV_ElementType *sourcePtr, int *locationPtr
)
{
    int
...
#define access(index)\
    *(rtFLOATUV_CPD_Array (targetCPD) + index)

/***** Make sure the target is a float set uvector ... *****/
    if ((targetCPD->RType () != RTYPE_C_FloatUV) ||
	!UV_CPD_IsASetUV (targetCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtFLOATUV_ScalarLocateOrAdd:  Target UVector Must Be A Float Set"
    );
 ...
 *****/
	...
	/*** Shift the container tail to add room ... ***/
	rtFLOATUV_CPD_Element (targetCPD) =
	    rtFLOATUV_CPD_Array (targetCPD) + addAt;
	...
	/*** insert the value ***/
	*(rtFLOATUV_CPD_Array (targetCPD) + addAt) = value;
...
/*---------------------------------------------------------------------------
 *****  Routine to lookup values in a float set uvector.
 *
 *  Arguments:
 *	sourceCPD	- a standard CPD for the float set uvector to be
 *			  be searched.
 *	keyCPD		- a standard CPD for the float set uvector containing
 ...
 *
 *****/
PublicFnDef void rtFLOATUV_Lookup (
    M_CPD		*sourceCPD,
    M_CPD		*keyCPD,
...
/***** Make sure both the source and key are float set uvectors ... *****/
    if ((sourceCPD->RType () != RTYPE_C_FloatUV) ||
	!UV_CPD_IsASetUV (sourceCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtFLOATUV_Lookup:  Source UVector Must Be A Float Set"
    );

    if ((keyCPD->RType () != RTYPE_C_FloatUV) ||
        !UV_CPD_IsASetUV (keyCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtFLOATUV_Lookup:  Key UVector Must Be A Float Set"
    );

/***** Align and validate source and key uvectors ... *****/
    rtFLOATUV_Align (sourceCPD);
    rtFLOATUV_Align (keyCPD);
    if (sourceCPD->ReferenceDoesntName (UV_CPx_RefPToken, keyCPD, UV_CPx_RefPToken)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtFLOATUV_Lookup:  Referential Inconsistency"
    );
...
/*---------------------------------------------------------------------------
 *****  Routine to lookup a value in a float set uvector.
 *
 *  Arguments:
 *	sourceCPD	- a standard CPD for the float set uvector to be
 *			  be searched.
 ...
 *
 *****/
PublicFnDef bool rtFLOATUV_ScalarLookup (
    M_CPD			*sourceCPD,
...
#define access(index)\
    *(rtFLOATUV_CPD_Array (sourceCPD) + index)

/***** Make sure the source is a float set uvector ... *****/
    if ((sourceCPD->RType () != RTYPE_C_FloatUV) ||
	!UV_CPD_IsASetUV (sourceCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
...
PrivateFnDef int PrintElement (M_CPD *cpd) {
    return IO_printf ("%g", *rtFLOATUV_CPD_Element (cpd));
}

...
UV_DefineTypeConversionDM (AsDoubleUVDM, rtFLOATUV_AsDoubleUV)

UV_DefineTypeConversionDM (AsIntUVDM, rtFLOATUV_AsIntUV)

UV_DefineAtDM (AtDM, rtFLOATUV_LCExtract, rtFLOATUV_UVExtract)

UV_DefineAtPutDM (AtPutDM, rtFLOATUV_LCAssign, rtFLOATUV_UVAssign)
...
UV_DefineAtRefPutDM (
    ...
    rtFLOATUV_RFAssign,
    rtFLOATUV_ElementType,
    IOBJ_NumericIObjectValue (parameterArray[1], rtFLOATUV_ElementType)
)

UV_DefineAtPutScalarDM (
    AtPutScalarDM,
    rtFLOATUV_LCAssignScalar,
    rtFLOATUV_UVAssignScalar,
    rtFLOATUV_ElementType,
    IOBJ_NumericIObjectValue (parameterArray[1], rtFLOATUV_ElementType)
)

...
IOBJ_DefineUnaryMethod (AtCurrentDM)
{
	...
	*rtFLOATUV_CPD_Element (RTYPE_QRegisterCPD (self))
    );
}
...
IOBJ_DefineMethod (AtCurrentPutDM) {
    ...
    *rtFLOATUV_CPD_Element (pCPD) = IOBJ_NumericIObjectValue (parameterArray[0], float);
...
/***** Sets values for all the elements in the uvector - from 0 to n *****/
{
    ...
    rtFLOATUV_ElementType
	*ptr = rtFLOATUV_CPD_Array (cpd);

    for (i=0; i<size; *ptr++ = (rtFLOATUV_ElementType)(i + .1), i++);
...
IOBJ_DefineUnaryMethod (ToSetUVDM) {
    ...
    rtFLOATUV_ToSetUV (IOBJ_ScratchPad, RTYPE_QRegisterCPD (self), &resultCPD, &refuvCPD);
...
IOBJ_DefineMethod (LocateOrAddDM) {
    ...
    rtFLOATUV_LocateOrAdd (targetCPD, sourceCPD, &locationsLinkC, &addedLinkC);
    if (addedLinkC) {
	IO_printf ("\nAdded LinkC:\n");
...
IOBJ_DefineMethod (ScalarLocateOrAddDM) {
    M_CPD *targetCPD = RTYPE_QRegisterCPD (self);
    rtFLOATUV_ElementType key = (rtFLOATUV_ElementType)IOBJ_IObjectValueAsInt (
	parameterArray[0]
    );

    int location;
    if (rtFLOATUV_ScalarLocateOrAdd (targetCPD, &key, &location))
...
IOBJ_DefineMethod (LookupDM) {
    ...
    rtFLOATUV_Lookup (sourceCPD, keyCPD, lookupCase, &locationsLinkC, &locatedLinkC);
    if (locatedLinkC) {
	IO_printf ("\nLocated LinkC:\n");
...
IOBJ_DefineMethod (ScalarLookupDM) {
    ...
    rtFLOATUV_ElementType key =
	(rtFLOATUV_ElementType)IOBJ_IObjectValueAsInt (parameterArray[0]);

    if (rtFLOATUV_ScalarLookup (sourceCPD, &key, lookupCase, &location))
...
RTYPE_DefineHandler(rtFLOATUV_Handler) {
	...
	M_RTD *rtd = iArgList.arg<M_RTD*>();
	    ...
	    M_RTD_HandleMaker		(rtd) = &rtFLOATUV_Handle::Maker;
	...
	UV_Print (rtFLOATUV_Align (iArgList.arg<M_CPD*>()), false, PrintElement);
        break;
    case RTYPE_RPrintObject:
	UV_Print (rtFLOATUV_Align (iArgList.arg<M_CPD*>()), true, PrintElement);
        break;
    case RTYPE_DoLCExtract:
        resultCPD   = iArgList.arg<M_CPD **>();
	sourceCPD   = iArgList.arg<M_CPD*>();
	linkc	    = iArgList.arg<rtLINK_CType *>();
	*resultCPD  = rtFLOATUV_LCExtract (sourceCPD, linkc);
        break;
    default:
        return -1;
    }
...
FAC_DefineFacility
{
    ...
    {
    FAC_FDFCase_FacilityIdAsString (rtFLOATUV);
...
/************************** SOURCE FILE HISTORY: ************************/
/************************************************************************
  RTfloatuv.c 1 replace /users/mjc/system
  860323 17:19:23 mjc New U-Vector representation types

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 2 replace /users/mjc/system
  860410 19:19:19 mjc Added P-Token and standard printer

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 3 replace /users/mjc/system
  860414 12:29:46 mjc Delete 'Browser' initialization from M_InitializeMData case

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 4 replace /users/mjc/system
  860420 18:49:17 mjc Replace for hjb to change 'M_{StandardCPD,NewSPadContainer}' calls

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 5 replace /users/jad
  860421 18:32:50 jad now uses io module

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 6 replace /users/mjc/system
  860422 22:55:32 mjc Return to library for 'hjb'

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 7 replace /users/hjb/system
  860423 00:35:46 hjb done updating calls to 'StandardCPD' & 'NewSPadC..'

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 8 replace /users/mjc/system
  860504 18:45:25 mjc Release uvectors

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 9 replace /users/mjc/system
  860521 11:10:07 mjc Release for coordination with jck

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 10 replace /users/mjc/system
  860530 14:36:47 mjc Added a referential p-token to the creation protocol for all u-vectors

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 11 replace /users/mjc/system
  860530 17:50:35 mjc Added 'RF{Extract,Assign}' routines

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 12 replace /users/mjc/system
  860615 19:01:02 mjc Eliminated 'ElementVStore' handler service case

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 13 replace /users/mjc/system
  860617 15:38:52 mjc Declared save routine

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 14 replace /users/jad/system
  860717 19:27:07 jad implemented rtFLOATUV_Distribute

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 15 replace /users/mjc/system
  861015 23:52:51 lcn Release of sorting, grouping, ranking, and formating primitives

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 16 replace /users/mjc/system
  861016 18:32:23 jad changed the uvector type conversion routines to accept the new
ref ptoken as an argument

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 17 replace /users/mjc/system
  861020 10:38:16 mjc Fixed transposed reference arguments in range case of 'PartitionedPartition'

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 18 replace /users/jad/system
  870429 16:51:26 jad implemented set operations

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 19 replace /users/jad/system
  870501 18:52:58 jad make sure to enable modifications
before turning off the IsASet bit

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 20 replace /users/mjc/translation
  870524 09:32:45 mjc Shortened all identifiers to <= 31 characters

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 21 replace /users/mjc/system
  870526 16:21:09 mjc Corrected 'intializeAdditionConstructors' (note missing 'i') mis-spelling

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 22 replace /users/jck/system
  871007 13:34:25 jck Added a marking function for the global garbage collector

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 23 replace /users/jck/system
  890222 15:16:12 jck Implemented corruption detection mechanism

 ************************************************************************/
/************************************************************************
  RTfloatuv.c 24 replace /users/m2/backend
  890927 15:01:18 m2 Changed major and minor variables because of SUN macro confilct, dummyiobj added

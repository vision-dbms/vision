/*****  Unsigned-64 U-Vector Representation Type Handler  *****/

/***************************
 *****  Facility Name  *****
 ***************************/

#define FacilityName rtU64UV
...
DEFINE_CONCRETE_RTT (rtU64UV_Handle);

 ...
 *
 *****/
PublicFnDef M_CPD *__cdecl rtU64UV_New (
...
/*****  Create and initialize the XIntUV  *****/
    return UV_New (
	RTYPE_C_Unsigned64UV, pPPT, pRPT, sizeof (rtU64UV_ElementType), initFn, initFnAP
    );
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_New (rtPTOKEN_Handle *pPPT, M_CPD *pRPTRef, int xRPTRef) {
    return UV_New (
	RTYPE_C_Unsigned64UV,
	pPPT,
	pRPTRef,
	xRPTRef,
	sizeof (rtU64UV_ElementType),
 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_New (rtPTOKEN_Handle *pPPT, rtPTOKEN_Handle *pRPT) {
    return UV_New (
	RTYPE_C_Unsigned64UV,
	pPPT,
	pRPT,
 ...
 *
 *****/
PrivateFnDef M_CPD *rtU64UV_Align (M_CPD *cpd) {
/*****  Validate Argument R-Type  *****/
    RTYPE_MustBeA ("rtU64UV_Align", cpd->RType (), RTYPE_C_Unsigned64UV);

/*****  Align Positionally  *****/
    static_cast<rtU64UV_Handle*>(cpd->containerHandle ())->align ();
...
/*****  If the source is empty, ... *****/
	...
	*resultCPD = rtU64UV_New (pResultPPT, sourceCPD, UV_CPx_RefPToken);
    }
/*****  ... otherwise, ...  *****/
    else {
    /*****  ... sort the source, ... *****/
	rtU64UV_ElementType const *const pSourceArray = rtU64UV_CPD_Array (sourceCPD);
	...
	unsigned int const *const pSortLimit = pSortArray + sSource;
        ...
        UTIL_Unsigned64QSort (pSourceArray, pSortArray, sSource, false);
	...
	*resultCPD = rtU64UV_New (pResultPPT, sourceCPD, UV_CPx_RefPToken);
	rtU64UV_ElementType *const pResultArray = rtU64UV_CPD_Array (*resultCPD);
 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_Distribute (M_CPD *refuvCPD, M_CPD *sourceuvCPD) {
    ...
    return rtU64UV_UVAssign (rtU64UV_New (pPPT, pRPT), refuvCPD, sourceuvCPD);
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_LCExtract (M_CPD *sourceCPD, rtLINK_CType *linkConstructor) {
/*****
 *  Align the link constructor and validate its applicability as an extraction
 *  subscript for 'source'
 *****/
    linkConstructor->AlignForExtract (rtU64UV_Align (sourceCPD), UV_CPx_PToken);
...
/*---------------------------------------------------------------------------
 *****  Internal 'rtU64UV_New' initialization routine to extract a
...
PrivateFnDef void InitializeExtractedUV (
	...
	*ip, *il, index,
    ...
    for (source = rtU64UV_CPD_Array (sourceCPD),
	 tp = rtU64UV_CPD_Array (extractedUVCPD),
	 il =
	     UV_CPD_ElementCount (refuvCPD) +
	         (ip = rtREFUV_CPD_Array (refuvCPD));
	 ip < il;
	 *tp++ = (index = *ip++) >= iRefNil ? VkUnsigned64::Zero () : source[index]);
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_UVExtract (M_CPD *sourceCPD, M_CPD *refuvCPD) {
/*****
 * Align 'source' and 'refuv' and validate 'refuv's applicability as an
 * extraction subscript for 'source'.
 *****/
    rtREFUV_AlignForExtract (rtU64UV_Align (sourceCPD), UV_CPx_PToken, refuvCPD);
...
/*****  Extract and return the requested values  *****/
    ...
    rtPTOKEN_Handle::Reference pRPT (static_cast<rtUVECTOR_Handle*>(sourceCPD->containerHandle ())->rptHandle ());
    return rtU64UV_New (pPPT, pRPT, InitializeExtractedUV, sourceCPD, refuvCPD);
}

 ...
 *
 *****/
PublicFnDef void rtU64UV_RFExtract (
 ...
 *****/
	...
	referenceAddr, rtU64UV_Align (sourceCPD), UV_CPx_PToken
    );
...
/*****  Extract and return the requested values  *****/
    *resultAddr = (
    ...
    ? VkUnsigned64::Zero ()
    : rtU64UV_CPD_Array (sourceCPD)[element];
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_UVAssign (
    M_CPD *targetCPD, M_CPD *refuvCPD, M_CPD *sourceCPD
)
{
    rtU64UV_ElementType		*target, *sp;
    int				*ip, *il;
...
/*****  Do the assignment and return  *****/
    ...
    for (target = rtU64UV_CPD_Array (targetCPD),
	 sp = rtU64UV_CPD_Array (sourceCPD),
	 il =
	     UV_CPD_ElementCount (refuvCPD) +
	         (ip = rtREFUV_CPD_Array (refuvCPD));
	 ip < il;
 ...
 *
 *****/
PublicFnDef void rtU64UV_RFAssign (
    M_CPD *targetCPD, rtREFUV_TypePTR_Reference reference, rtU64UV_ElementType *value
)
{
/*****
 ...
 *****/
    rtREFUV_AlignAndValidateRef (
	reference, rtU64UV_Align (targetCPD), UV_CPx_PToken
    );

/*****  Do the assignment and return  *****/
    targetCPD->EnableModifications ();
    UV_CPD_IsASetUV (targetCPD) = false;
    rtU64UV_CPD_Array (targetCPD)
 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_LCAssignScalar (
    M_CPD *targetCPD, rtLINK_CType *linkConstructor, rtU64UV_ElementType *valuePtr
) {
/*****  Align 'target' and perform the assignment  *****/
    return UV_LCAssignScalar (
	rtU64UV_Align (targetCPD), linkConstructor, (V::pointer_t)valuePtr
    );
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_UVAssignScalar (
    M_CPD *targetCPD, M_CPD *refuvCPD, rtU64UV_ElementType *valuePtr
)
{
    rtU64UV_ElementType		*target, value = *valuePtr;
    int				*ip, *il;
 ...
 *****/
	...
	rtU64UV_Align (targetCPD), UV_CPx_PToken, refuvCPD
    );
...
/*****  Do the assignment and return  *****/
    ...
    for (
	target = rtU64UV_CPD_Array (targetCPD),
 ...
 *
 *****/
PublicFnDef M_CPD *rtU64UV_PartitndSortIndices (
...
/*****  Align and validate parameters...  *****/
    partition->AlignForDistribute (rtU64UV_Align (values), UV_CPx_PToken);

/*****  Create the sort indices...  *****/
    M_CPD *result = rtREFUV_New (partition->PPT (), partition->PPT ());

/*****  Initialize the traversal pointers...  *****/
    rtU64UV_ElementType const *valuesArray = rtU64UV_CPD_Array (values);
 ...
 *
 *****/
PublicFnDef void rtU64UV_PartitndPartition (
 ...
 *---------------------------------------------------------------------------*/
    ...
    rtU64UV_ElementType previousValue, currentValue;
...
/*****  Initialize the traversal pointers...  *****/
    rtU64UV_ElementType const *valuesPtr = rtU64UV_CPD_Array (values);
 ...
 *
 *****/
PublicFnDef void rtU64UV_LocateOrAdd (
    M_CPD*			targetCPD,
    M_CPD*			sourceCPD,
...
/***** Make sure both the source and target are unsigned-64 set uvectors ... *****/
    if ((sourceCPD->RType () != RTYPE_C_Unsigned64UV) ||
	!UV_CPD_IsASetUV (sourceCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtU64UV_LocateOrAdd:  Source UVector Must Be An Extended Integer Set"
    );

    if ((targetCPD->RType () != RTYPE_C_Unsigned64UV) ||
	!UV_CPD_IsASetUV (targetCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtU64UV_LocateOrAdd:  Target UVector Must Be An Extended Integer Set"
    );
...
/***** Align and validate source and target uvectors ... *****/
	...
	EC__InternalInconsistency,
	"rtU64UV_LocateOrAdd:  Referential Inconsistency"
    );
...
/***** Allocate space for the new target values ... *****/
    ...
    resultStart = resultp = (rtU64UV_ElementType *) UTIL_Malloc (
	largestSize * sizeof (rtU64UV_ElementType)
    );

/***** Begin the traversal ... *****/
    resultCount = 0;
    srcp = rtU64UV_CPD_Array (sourceCPD);
    srcpl = srcp + UV_CPD_ElementCount (sourceCPD);
    srcPos = 0;
    trgp = rtU64UV_CPD_Array (targetCPD);
...
/***** If values were added ... *****/
	...
	rtU64UV_CPD_Element (targetCPD) =
	    rtU64UV_CPD_Array (targetCPD) + origTargetCount;
 ...
 *
 *****/
PublicFnDef bool rtU64UV_ScalarLocateOrAdd (
    M_CPD *targetCPD, rtU64UV_ElementType *sourcePtr, int *locationPtr
)
{
    int
...
#define access(index)\
    *(rtU64UV_CPD_Array (targetCPD) + index)
...
/***** Make sure the target is a integer set uvector ... *****/
	...
	EC__InternalInconsistency,
	"rtU64UV_ScalarLocateOrAdd:  Target UVector Must Be An Integer Set"
    );
 ...
 *****/
	...
	/*** Shift the container tail to add room ... ***/
	rtU64UV_CPD_Element (targetCPD)=rtU64UV_CPD_Array (targetCPD) + addAt;
	...
	/*** insert the value ***/
	*(rtU64UV_CPD_Array (targetCPD) + addAt) = value;
 ...
 *
 *****/
PublicFnDef void rtU64UV_Lookup (
    M_CPD			*sourceCPD,
    M_CPD			*keyCPD,
...
\
	...
	default:\
	    ERR_SignalFault (\
		EC__InternalInconsistency,\
		"rtU64UV_Lookup: Unknown Comparison Operation"\
	    );\
	    break;\
	}\
    }\
...
/***** Make sure both the source and key are unsigned-64 set uvectors ... *****/
    if ((sourceCPD->RType () != RTYPE_C_Unsigned64UV) ||
	!UV_CPD_IsASetUV (sourceCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtU64UV_Lookup:  Source UVector Must Be An Integer Set"
    );

    if ((keyCPD->RType () != RTYPE_C_Unsigned64UV) ||
        !UV_CPD_IsASetUV (keyCPD)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtU64UV_Lookup:  Key UVector Must Be An Integer Set"
    );

/***** Align and validate source and key uvectors ... *****/
    rtU64UV_Align (sourceCPD);
    rtU64UV_Align (keyCPD);
    if (sourceCPD->ReferenceDoesntName (UV_CPx_RefPToken, keyCPD, UV_CPx_RefPToken)
    ) ERR_SignalFault (
	EC__InternalInconsistency,
	"rtU64UV_Lookup:  Referential Inconsistency"
    );
 ...
 *
 *****/
PublicFnDef bool rtU64UV_ScalarLookup (
    M_CPD			*sourceCPD,
...
#define access(index)\
    *(rtU64UV_CPD_Array (sourceCPD) + index)
...
PrivateFnDef int PrintElement (M_CPD *cpd) {
    rtU64UV_ElementType const &rElement = *rtU64UV_CPD_Element (cpd);
    return IO_printf ("%u.%u", rElement[0], rElement[1]);
}

...
UV_DefineAtDM (AtDM, rtU64UV_LCExtract, rtU64UV_UVExtract)

UV_DefineAtPutDM (AtPutDM, rtU64UV_LCAssign, rtU64UV_UVAssign)
...
IOBJ_DefineUnaryMethod (ToSetUVDM) {
    ...
    rtU64UV_ToSetUV (IOBJ_ScratchPad, RTYPE_QRegisterCPD (self), &resultCPD, &refuvCPD);
...
IOBJ_DefineMethod (LocateOrAddDM) {
    ...
    rtU64UV_LocateOrAdd (targetCPD, sourceCPD, &locationsLinkC, &addedLinkC);
    if (addedLinkC) {
	IO_printf ("\nAdded LinkC:\n");
...
IOBJ_DefineMethod (LookupDM) {
    ...
    rtU64UV_Lookup (sourceCPD, keyCPD, lookupCase, &locationsLinkC, &locatedLinkC);
    if (locatedLinkC) {
	IO_printf ("\nLocated LinkC:\n");
...
RTYPE_DefineHandler(rtU64UV_Handler) {
    ...
    case RTYPE_InitializeMData: {
	    ...
	    M_RTD_HandleMaker		(rtd) = &rtU64UV_Handle::Maker;
	...
	UV_Print (rtU64UV_Align (iArgList.arg<M_CPD*>()), false, PrintElement);
        break;
    case RTYPE_RPrintObject:
	UV_Print (rtU64UV_Align (iArgList.arg<M_CPD*>()), true, PrintElement);
        break;
    case RTYPE_DoLCExtract:
        resultCPD   = iArgList.arg<M_CPD**>();
	sourceCPD   = iArgList.arg<M_CPD*>();
	linkc	    = iArgList.arg<rtLINK_CType*>();
	*resultCPD  = rtU64UV_LCExtract (sourceCPD, linkc);
        break;
    default:
        return -1;
    }
...
FAC_DefineFacility {
    ...
    FAC_FDFCase_FacilityIdAsString (rtU64UV);
    FAC_FDFCase_FacilityDescription ("Int U-Vector Representation Type Handler");
    default:
        break;
    }
}

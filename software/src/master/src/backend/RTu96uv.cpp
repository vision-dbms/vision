/*****  Unsigned-96 U-Vector Representation Type Handler  *****/

/***************************
 *****  Facility Name  *****
 ***************************/

#define FacilityName rtU96UV
...
DEFINE_CONCRETE_RTT (rtU96UV_Handle);

 ...
 *
 *****/
PublicFnDef void rtU96UV_ToSetUV (
...
/*****  ... otherwise, ...  *****/
    ...
    /*****  ... sort the source, ... *****/
	rtU96UV_ElementType const *const pSourceArray = rtU96UV_CPD_Array (sourceCPD);
	...
	unsigned int const *const pSortLimit = pSortArray + sSource;
        ...
        UTIL_Unsigned96QSort (pSourceArray, pSortArray, sSource, false);
	...
	*resultCPD = rtU96UV_New (pResultPPT, sourceCPD, UV_CPx_RefPToken);
	rtU96UV_ElementType *const pResultArray = rtU96UV_CPD_Array (*resultCPD);
 ...
 *
 *****/
PublicFnDef M_CPD *rtU96UV_LCExtract (M_CPD *sourceCPD, rtLINK_CType *linkConstructor) {
/*****
...
/*---------------------------------------------------------------------------
 *****  Internal 'rtU96UV_New' initialization routine to extract a
...
PrivateFnDef void InitializeExtractedUV (
	...
	*ip, *il, index,
    ...
    for (source = rtU96UV_CPD_Array (sourceCPD),
	 tp = rtU96UV_CPD_Array (extractedUVCPD),
	 il =
	     UV_CPD_ElementCount (refuvCPD) +
	         (ip = rtREFUV_CPD_Array (refuvCPD));
	 ip < il;
	 *tp++ = (index = *ip++) >= iRefNil ? VkUnsigned96::Zero () : source[index]);
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtU96UV_UVExtract (M_CPD *sourceCPD, M_CPD *refuvCPD) {
/*****
 ...
 *
 *****/
PublicFnDef void rtU96UV_RFExtract (
 ...
 *****/
	...
	referenceAddr, rtU96UV_Align (sourceCPD), UV_CPx_PToken
    );
 ...
 *
 *****/
PublicFnDef M_CPD *rtU96UV_UVAssign (M_CPD *targetCPD, M_CPD *refuvCPD, M_CPD *sourceCPD) {
    rtU96UV_ElementType		*target, *sp;
    int				*ip, *il;
...
/*****  Do the assignment and return  *****/
    ...
    for (target = rtU96UV_CPD_Array (targetCPD),
	 sp = rtU96UV_CPD_Array (sourceCPD),
	 il =
	     UV_CPD_ElementCount (refuvCPD) +
	         (ip = rtREFUV_CPD_Array (refuvCPD));
	 ip < il;
 ...
 *
 *****/
PublicFnDef void rtU96UV_RFAssign (
    M_CPD *targetCPD, rtREFUV_TypePTR_Reference reference, rtU96UV_ElementType *value
)
{
/*****
 ...
 *****/
    rtREFUV_AlignAndValidateRef (
	reference, rtU96UV_Align (targetCPD), UV_CPx_PToken
    );

/*****  Do the assignment and return  *****/
    targetCPD->EnableModifications ();
    UV_CPD_IsASetUV (targetCPD) = false;
    rtU96UV_CPD_Array (targetCPD)
 ...
 *
 *****/
PublicFnDef M_CPD *rtU96UV_LCAssignScalar (
    M_CPD *targetCPD, rtLINK_CType *linkConstructor, rtU96UV_ElementType *valuePtr
) {
/*****  Align 'target' and perform the assignment  *****/
    return UV_LCAssignScalar (
	rtU96UV_Align (targetCPD), linkConstructor, (V::pointer_t)valuePtr
    );
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtU96UV_UVAssignScalar (
    M_CPD *targetCPD, M_CPD *refuvCPD, rtU96UV_ElementType *valuePtr
)
{
    rtU96UV_ElementType		*target, value = *valuePtr;
    int				*ip, *il;
 ...
 *****/
	...
	rtU96UV_Align (targetCPD), UV_CPx_PToken, refuvCPD
    );
...
/*****  Do the assignment and return  *****/
    ...
    for (
	target = rtU96UV_CPD_Array (targetCPD),
 ...
 *
 *****/
PublicFnDef M_CPD *rtU96UV_PartitndSortIndices (
...
/*****  Initialize the traversal pointers...  *****/
    rtU96UV_ElementType const *valuesArray = rtU96UV_CPD_Array (values);
 ...
 *
 *****/
PublicFnDef void rtU96UV_PartitndPartition (
 ...
 *---------------------------------------------------------------------------*/
    ...
    rtU96UV_ElementType previousValue, currentValue;
 ...
 *
 *****/
PublicFnDef void rtU96UV_LocateOrAdd (
    M_CPD*			targetCPD,
    M_CPD*			sourceCPD,
...
/***** Make sure both the source and target are unsigned-96 set uvectors ... *****/
	...
	EC__InternalInconsistency,
	"rtU96UV_LocateOrAdd:  Target UVector Must Be An Extended Integer Set"
    );
...
/***** Align and validate source and target uvectors ... *****/
	...
	EC__InternalInconsistency,
	"rtU96UV_LocateOrAdd:  Referential Inconsistency"
    );
...
/***** Allocate space for the new target values ... *****/
    ...
    resultStart = resultp = (rtU96UV_ElementType *) UTIL_Malloc (
	largestSize * sizeof (rtU96UV_ElementType)
    );

/***** Begin the traversal ... *****/
    resultCount = 0;
    srcp = rtU96UV_CPD_Array (sourceCPD);
    srcpl = srcp + UV_CPD_ElementCount (sourceCPD);
    srcPos = 0;
    trgp = rtU96UV_CPD_Array (targetCPD);
...
/***** If values were added ... *****/
	...
	rtU96UV_CPD_Element (targetCPD) =
	    rtU96UV_CPD_Array (targetCPD) + origTargetCount;
 ...
 *
 *****/
PublicFnDef bool rtU96UV_ScalarLocateOrAdd (
    M_CPD *targetCPD, rtU96UV_ElementType *sourcePtr, int *locationPtr
)
{
    int
...
#define access(index)\
    *(rtU96UV_CPD_Array (targetCPD) + index)
...
/***** Make sure the target is a integer set uvector ... *****/
	...
	EC__InternalInconsistency,
	"rtU96UV_ScalarLocateOrAdd:  Target UVector Must Be An Integer Set"
    );
 ...
 *****/
	...
	/*** Shift the container tail to add room ... ***/
	rtU96UV_CPD_Element (targetCPD)=rtU96UV_CPD_Array (targetCPD) + addAt;
 ...
 *
 *****/
PublicFnDef void rtU96UV_Lookup (
    M_CPD			*sourceCPD,
    M_CPD			*keyCPD,
...
\
	...
	default:\
	    ERR_SignalFault (\
		EC__InternalInconsistency,\
		"rtU96UV_Lookup: Unknown Comparison Operation"\
	    );\
	    break;\
	}\
    }\
 ...
 *
 *****/
PublicFnDef bool rtU96UV_ScalarLookup (
    M_CPD			*sourceCPD,
...
PrivateFnDef int PrintElement (M_CPD *cpd) {
    rtU96UV_ElementType const &rElement = *rtU96UV_CPD_Element (cpd);
    return IO_printf ("%u.%u.%u", rElement[0], rElement[1], rElement[2]);
}

...
IOBJ_DefineUnaryMethod (ToSetUVDM) {
    ...
    rtU96UV_ToSetUV (IOBJ_ScratchPad, RTYPE_QRegisterCPD (self), &resultCPD, &refuvCPD);
...
IOBJ_DefineMethod (LocateOrAddDM) {
    ...
    rtU96UV_LocateOrAdd (targetCPD, sourceCPD, &locationsLinkC, &addedLinkC);
    if (addedLinkC) {
	IO_printf ("\nAdded LinkC:\n");
...
IOBJ_DefineMethod (LookupDM) {
    ...
    rtU96UV_Lookup (sourceCPD, keyCPD, lookupCase, &locationsLinkC, &locatedLinkC);
    if (locatedLinkC) {
	IO_printf ("\nLocated LinkC:\n");
...
RTYPE_DefineHandler(rtU96UV_Handler) {
    ...
    case RTYPE_InitializeMData:{
	    ...
	    M_RTD_HandleMaker		(rtd) = &rtU96UV_Handle::Maker;
	...
	UV_Print (rtU96UV_Align (iArgList.arg<M_CPD*>()), false, PrintElement);
        break;
    case RTYPE_RPrintObject:
	UV_Print (rtU96UV_Align (iArgList.arg<M_CPD*>()), true, PrintElement);
        break;
    case RTYPE_DoLCExtract:
        resultCPD   = iArgList.arg<M_CPD**>();
	sourceCPD   = iArgList.arg<M_CPD*>();
	linkc	    = iArgList.arg<rtLINK_CType*>();
	*resultCPD  = rtU96UV_LCExtract (sourceCPD, linkc);
        break;
    default:
        return -1;
    }
...
FAC_DefineFacility {
    ...
    FAC_FDFCase_FacilityIdAsString (rtU96UV);
    FAC_FDFCase_FacilityDescription ("Unsigned-96 U-Vector Representation Type Handler");
    default:
        break;
    }
}

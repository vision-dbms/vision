...
echo `$Tools/vpooladmin $serverArg -flushWorkers` >> /dev/null
set TotalWorkersResult = `$Tools/vpooladmin $serverArg -totalWorkers`
echo -n "FlushWorkers Test       :"
echo -n "FlushWorkers Test       :" >> $LogFile

if ($TotalWorkersResult == 0) then
   echo "PASSED"
   echo "PASSED" >> $LogFile
else
   echo "FAILED"
   echo "FAILED" >> $LogFile
   echo "Worker Count Not 0 after Flush:" $TotalWorkersResult >> $LogFile
endif

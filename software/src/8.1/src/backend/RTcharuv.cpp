/*****  Char U-Vector Representation Type Handler  *****/

/***************************
 *****  Facility Name  *****
 ***************************/

#define FacilityName rtCHARUV
...
DEFINE_CONCRETE_RTT (rtCHARUV_Handle);

...
/*---------------------------------------------------------------------------
 *****  Routine to positionally align a CharUV.
 *
 *  Argument:
 *	cpd			- a CPD for the CharUV to be aligned.
 *
 *  Returns:
 *	'cpd'
 *
 ...
 *
 *****/
PublicFnDef void rtCHARUV_ToSetUV (
...
/*****  ... otherwise, ...  *****/
    ...
    /*****  ... sort the source, ... *****/
	rtCHARUV_ElementType const *const pSourceArray = rtCHARUV_CPD_Array (sourceCPD);
	...
	unsigned int const *const pSortLimit = pSortArray + sSource;
        ...
        UTIL_CharQSort (pSourceArray, pSortArray, sSource, false);

    /*****  ... create the source->set reference map (using the wrong codomain), ... *****/
	...
	*resultCPD = rtCHARUV_New (pResultPPT, sourceCPD, UV_CPx_RefPToken);
	rtCHARUV_ElementType *const pResultArray = rtCHARUV_CPD_Array (*resultCPD);
 ...
 *
 *  Arguments:
 *	sourceCPD		- a standard CPD for the CharUV from which
 ...
 *
 *  Returns:
 *	A standard CPD for the CharUV containing the extracted elements.
 *
 *  Notes:
 ...
 *
 *****/
PublicFnDef M_CPD *rtCHARUV_LCExtract (M_CPD *sourceCPD, rtLINK_CType *linkConstructor) {
/*****
...
/*---------------------------------------------------------------------------
 *****  Internal 'rtCHARUV_New' initialization routine to extract a
 *****  collection of elements from a CharUV.
 *
 *  Arguments:
 *	extractedUVCPD		- an initialized standard CPD for the
 *				  CharUV being extracted.
 *	nelements		- the number of elements in the CharUV
 *				  being extracted.
...
PrivateFnDef void InitializeExtractedUV (
	...
	*ip, *il, index,
    ...
    for (
	source = rtCHARUV_CPD_Array (sourceCPD),
	tp = rtCHARUV_CPD_Array (extractedUVCPD),
 ...
 *
 *****/
PublicFnDef M_CPD *rtCHARUV_UVExtract (M_CPD *sourceCPD, M_CPD *refuvCPD) {
/*****
 ...
 *
 *****/
PublicFnDef void rtCHARUV_RFExtract (
 ...
 *****/
	...
	referenceAddr, rtCHARUV_Align (sourceCPD), UV_CPx_PToken
    );
 ...
 *
 *****/
PublicFnDef M_CPD *rtCHARUV_LCAssign (
 ...
 *
 *****/
PublicFnDef M_CPD *rtCHARUV_UVAssign (
...
/*****  Do the assignment and return  *****/
    ...
    for (target = rtCHARUV_CPD_Array (targetCPD),
	 sp = rtCHARUV_CPD_Array (sourceCPD),
	 il =
 ...
 *
 *****/
PublicFnDef void rtCHARUV_RFAssign (
    M_CPD *			targetCPD,
 ...
 *****/
    rtREFUV_AlignAndValidateRef (
	reference, rtCHARUV_Align (targetCPD), UV_CPx_PToken
    );

/*****  Do the assignment and return  *****/
    targetCPD->EnableModifications ();
    UV_CPD_IsASetUV (targetCPD) = false;
    rtCHARUV_CPD_Array (targetCPD)
 ...
 *
 *****/
PublicFnDef M_CPD *rtCHARUV_LCAssignScalar (
    M_CPD *targetCPD, rtLINK_CType *linkConstructor, rtCHARUV_ElementType *valuePtr
) {
/*****  Align 'target' and perform the assignment  *****/
    return UV_LCAssignScalar (
	rtCHARUV_Align (targetCPD), linkConstructor, valuePtr
    );
}

 ...
 *
 *****/
PublicFnDef M_CPD *rtCHARUV_UVAssignScalar (
    M_CPD *targetCPD, M_CPD *refuvCPD, rtCHARUV_ElementType *valuePtr
)
{
    rtCHARUV_ElementType	*target, value = *valuePtr;
    int				*ip, *il;
 ...
 *****/
	...
	rtCHARUV_Align (targetCPD), UV_CPx_PToken, refuvCPD
    );
 ...
 *
 *****/
PublicFnDef M_CPD *rtCHARUV_PartitndSortIndices (
...
/*****  Initialize the traversal pointers...  *****/
    rtCHARUV_ElementType const *valuesArray = rtCHARUV_CPD_Array (values);
 ...
 *
 *****/
PublicFnDef void rtCHARUV_PartitndPartition (
 ...
 *---------------------------------------------------------------------------*/
    ...
    rtCHARUV_ElementType previousValue, currentValue;
...
/*---------------------------------------------------------------------------
 *****  Routine to locate or add a set of values to a character set uvector.
 *
 *  Arguments:
 *	targetCPD	- a standard CPD for a character set uvector for the
 *                        uvector to be added to.
 *	sourceCPD	- a standard CPD for a character set uvector containing
 ...
 *
 *****/
PublicFnDef void rtCHARUV_LocateOrAdd (
    M_CPD*			targetCPD,
    M_CPD*			sourceCPD,
...
/***** Allocate space for the new target values ... *****/
    ...
    resultStart = resultp = (rtCHARUV_ElementType *) UTIL_Malloc (
	largestSize * sizeof (rtCHARUV_ElementType)
    );

/***** Begin the traversal ... *****/
    resultCount = 0;
    srcp = rtCHARUV_CPD_Array (sourceCPD);
    srcpl = srcp + UV_CPD_ElementCount (sourceCPD);
    srcPos = 0;
    trgp = rtCHARUV_CPD_Array (targetCPD);
...
/***** If values were added ... *****/
	...
	rtCHARUV_CPD_Element (targetCPD) =
	    rtCHARUV_CPD_Array (targetCPD) + origTargetCount;
...
/*---------------------------------------------------------------------------
 *****  Routine to locate or add a value to a character set uvector.
 *
 *  Arguments:
 *	targetCPD	- a standard CPD for a character set uvector for the
 *                        uvector to be added to.
 ...
 *
 *****/
PublicFnDef bool rtCHARUV_ScalarLocateOrAdd (
    M_CPD			*targetCPD,
...
#define access(index)\
    *(rtCHARUV_CPD_Array (targetCPD) + index)
 ...
 *****/
	...
	/*** Shift the container tail to add room ... ***/
	rtCHARUV_CPD_Element (targetCPD) = rtCHARUV_CPD_Array (targetCPD) + addAt;
...
/*---------------------------------------------------------------------------
 *****  Routine to lookup values in a character set uvector.
 *
 *  Arguments:
 *	sourceCPD	- a standard CPD for the character set uvector to be
 *			  be searched.
 *	keyCPD		- a standard CPD for the character set uvector
 ...
 *
 *****/
PublicFnDef void rtCHARUV_Lookup (
    M_CPD		*sourceCPD,
    M_CPD		*keyCPD,
...
/*---------------------------------------------------------------------------
 *****  Routine to lookup a value in a character set uvector.
 *
 *  Arguments:
 *	sourceCPD	- a standard CPD for the character set uvector to be
 *			  be searched.
 ...
 *
 *****/
PublicFnDef bool rtCHARUV_ScalarLookup (
    M_CPD			*sourceCPD,
...
PrivateFnDef int PrintElement (M_CPD *cpd) {
    int	c = *rtCHARUV_CPD_Element (cpd);

    return IO_printf (isascii (c) && isprint (c) ? "%c" : "\\%x", c);
}

...
UV_DefineAtPutDM (AtPutDM, rtCHARUV_LCAssign, rtCHARUV_UVAssign)

UV_DefineAtRefDM (
    AtReferenceDM, rtCHARUV_RFExtract, rtCHARUV_ElementType, IOBJ_CharIObject
)
...
UV_DefineAtRefPutDM (
    ...
    rtCHARUV_RFAssign,
    rtCHARUV_ElementType,
    IOBJ_NumericIObjectValue (parameterArray[1], rtCHARUV_ElementType)
)

UV_DefineAtPutScalarDM (
    AtPutScalarDM,
    rtCHARUV_LCAssignScalar,
    rtCHARUV_UVAssignScalar,
    rtCHARUV_ElementType,
    IOBJ_NumericIObjectValue (parameterArray[1], rtCHARUV_ElementType)
)

...
{
    return IOBJ_CharIObject(*rtCHARUV_CPD_Element (RTYPE_QRegisterCPD (self)));
}
...
IOBJ_DefineMethod (AtCurrentPutDM) {
    ...
    *rtCHARUV_CPD_Element (pCPD) = IOBJ_NumericIObjectValue (parameterArray[0], char);
...
IOBJ_DefineUnaryMethod (ToSetUVDM) {
    ...
    rtCHARUV_ToSetUV (IOBJ_ScratchPad, RTYPE_QRegisterCPD (self), &resultCPD, &refuvCPD);
...
IOBJ_DefineMethod (LocateOrAddDM) {
    ...
    rtCHARUV_LocateOrAdd (targetCPD, sourceCPD, &locationsLinkC, &addedLinkC);
    if (addedLinkC) {
	IO_printf ("\nAdded LinkC:\n");
...
IOBJ_DefineMethod (ScalarLocateOrAddDM) {
    M_CPD *targetCPD = RTYPE_QRegisterCPD (self);
    rtCHARUV_ElementType key = (rtCHARUV_ElementType) IOBJ_NumericIObjectValue (
	parameterArray[0], rtCHARUV_ElementType
    );

    int location;
    if (rtCHARUV_ScalarLocateOrAdd (targetCPD, &key, &location))
...
IOBJ_DefineMethod (LookupDM) {
    ...
    rtLINK_LookupCase lookupCase = (rtLINK_LookupCase)IOBJ_IObjectValueAsInt (parameterArray[1]);

    rtLINK_CType *locationsLinkC, *locatedLinkC;
    rtCHARUV_Lookup (sourceCPD, keyCPD, lookupCase, &locationsLinkC, &locatedLinkC);
    if (locatedLinkC) {
	IO_printf ("\nLocated LinkC:\n");
...
IOBJ_DefineMethod (ScalarLookupDM) {
    ...
    rtCHARUV_ElementType key = (rtCHARUV_ElementType)
	    IOBJ_NumericIObjectValue (parameterArray[0], rtCHARUV_ElementType);

    if (rtCHARUV_ScalarLookup (sourceCPD, &key, lookupCase, &location))
...
RTYPE_DefineHandler(rtCHARUV_Handler) {
	...
	M_RTD *rtd = iArgList.arg<M_RTD*>();
	    ...
	    M_RTD_HandleMaker		(rtd) = &rtCHARUV_Handle::Maker;
	...
	*resultCPD  = rtCHARUV_LCExtract (sourceCPD, linkc);
        break;
    default:
        return -1;
    }
...
FAC_DefineFacility {
    ...
    FAC_FDFCase_FacilityIdAsString (rtCHARUV);
    FAC_FDFCase_FacilityDescription ("Char U-Vector Representation Type Handler");
    default:
        break;
    }
}
...
/************************** SOURCE FILE HISTORY: ************************/
/************************************************************************
  RTcharuv.c 1 replace /users/mjc/system
  860508 18:12:30 mjc Added 'character' u-vector

 ************************************************************************/
/************************************************************************
  RTcharuv.c 2 replace /users/mjc/system
  860509 10:25:39 mjc Changed printers to deal with non-printing characters

 ************************************************************************/
/************************************************************************
  RTcharuv.c 3 replace /users/mjc/system
  860530 14:36:41 mjc Added a referential p-token to the creation protocol for all u-vectors

 ************************************************************************/
/************************************************************************
  RTcharuv.c 4 replace /users/mjc/system
  860530 22:51:00 mjc Added 'RF{Extract/Assign}' operations

 ************************************************************************/
/************************************************************************
  RTcharuv.c 5 replace /users/mjc/system
  860617 15:38:44 mjc Declared save routine

 ************************************************************************/
/************************************************************************
  RTcharuv.c 6 replace /users/jad/system
  860718 11:23:44 jad added a Distribute Macro

 ************************************************************************/
/************************************************************************
  RTcharuv.c 7 replace /users/mjc/system
  861015 23:52:38 lcn Release of sorting, grouping, ranking, and formating primitives

 ************************************************************************/
/************************************************************************
  RTcharuv.c 8 replace /users/mjc/system
  861020 10:38:09 mjc Fixed transposed reference arguments in range case of 'PartitionedPartition'

 ************************************************************************/
/************************************************************************
  RTcharuv.c 9 replace /users/jad/system
  870429 16:52:32 jad implemented set operations

 ************************************************************************/
/************************************************************************
  RTcharuv.c 10 replace /users/jad/system
  870501 18:52:48 jad make sure to enable modifications
before turning off the IsASet bit

 ************************************************************************/
/************************************************************************
  RTcharuv.c 11 replace /users/mjc/translation
  870524 09:31:41 mjc Shortened all identifiers to <= 31 characters

 ************************************************************************/
/************************************************************************
  RTcharuv.c 12 replace /users/mjc/system
  870526 16:20:41 mjc Corrected 'intializeAdditionConstructors' (note missing 'i') mis-spelling

 ************************************************************************/
/************************************************************************
  RTcharuv.c 13 replace /users/jck/system
  871007 13:34:11 jck Added a marking function for the global garbage collector

 ************************************************************************/
/************************************************************************
  RTcharuv.c 14 replace /users/jck/system
  890222 15:12:29 jck Implemented corruption detection mechanism

 ************************************************************************/
/************************************************************************
  RTcharuv.c 15 replace /users/m2/backend
  890927 14:56:19 m2 Changed major and minor variables because of SUN macro confilct, dummyiobj added

 ...
 ***********************************
 *****                         *****
 *****  Vdb::IDatabaseManager  *****
 *****                         *****
 ***********************************
 ***********************************/

#define Vdb_IDatabaseManager

//DECLARE_VdbINTERFACE (IDatabase)

//VINTERFACE_TEMPLATE_INSTANTIATIONS (Vdb,IDatabaseManager)
...
VINTERFACE_TEMPLATE_EXPORTS (Vdb::IDatabaseManager)

namespace Vdb {
    // {AE469C86-05B5-4da5-9946-D2A8D23D9F21}
    VINTERFACE_TYPEINFO_DEFINITION (
	IDatabaseManager,
	0xae469c86, 0x5b5, 0x4da5, 0x99, 0x46, 0xd2, 0xa8, 0xd2, 0x3d, 0x9f, 0x21
    );

    //  Member Definitions
    VINTERFACE_MEMBERINFO_DEFINITION (IDatabaseManager, OpenDatabase, 0);
}

...
#define Vdb_IDatabase

//DECLARE_VdbINTERFACE (ITransaction)

//VINTERFACE_TEMPLATE_INSTANTIATIONS (Vdb,IDatabase)

namespace Vdb {
    class ITransaction;

    VINTERFACE_ROLE (IDatabase, IVUnknown)
	VINTERFACE_ROLE_3 (UpdateFrom, ITransaction*, IDatabase*, uuid_t const&);
    VINTERFACE_ROLE_END

    VINTERFACE (IDatabase, IVUnknown)
	VINTERFACE_METHOD_3 (UpdateFrom, ITransaction*, IDatabase*, uuid_t const&);
    VINTERFACE_END
}

VINTERFACE_TEMPLATE_EXPORTS (Vdb::IDatabase)

namespace Vdb {
    // {22C38BDB-278E-42c6-8F3E-9B9840787C2E}
    VINTERFACE_TYPEINFO_DEFINITION (
	IDatabase,
	0x22c38bdb, 0x278e, 0x42c6, 0x8f, 0x3e, 0x9b, 0x98, 0x40, 0x78, 0x7c, 0x2e
    );

    //  Member Definitions
    VINTERFACE_MEMBERINFO_DEFINITION (IDatabase, UpdateFrom, 0);
}

...
#define Vdb_IDatabaseUpdate

//VINTERFACE_TEMPLATE_INSTANTIATIONS (Vdb,IDatabaseUpdate)

namespace Vdb {
    VINTERFACE_ROLE (IDatabaseUpdate, IVUnknown)
//	VINTERFACE_ROLE_3 (UpdateFrom, ITransaction*, IDatabase*, uuid_t const&);
    VINTERFACE_ROLE_END

    VINTERFACE (IDatabaseUpdate, IVUnknown)
//	VINTERFACE_METHOD_3 (UpdateFrom, ITransaction*, IDatabase*, uuid_t const&);
    VINTERFACE_END
}

VINTERFACE_TEMPLATE_EXPORTS (Vdb::IDatabaseUpdate)

namespace Vdb {
    // {8B183832-63B0-46ea-ACDE-7F90DC834835}
    VINTERFACE_TYPEINFO_DEFINITION (
	IDatabaseUpdate,
	0x8b183832, 0x63b0, 0x46ea, 0xac, 0xde, 0x7f, 0x90, 0xdc, 0x83, 0x48, 0x35
    );
...
#define Vdb_ITransaction

//VINTERFACE_TEMPLATE_INSTANTIATIONS (Vdb,ITransaction)
...
VINTERFACE_TEMPLATE_EXPORTS (Vdb::ITransaction)

namespace Vdb {
    // {11B42250-D58D-4403-A143-5B61C3240C88}
    VINTERFACE_TYPEINFO_DEFINITION (
	ITransaction,
	0x11b42250, 0xd58d, 0x4403, 0xa1, 0x43, 0x5b, 0x61, 0xc3, 0x24, 0xc, 0x88
    );

    //  Member Definitions
    VINTERFACE_MEMBERINFO_DEFINITION (ITransaction, LogMessage, 0);
}

...
class VNetworkHandle : public Vca::VRolePlayer {
    DECLARE_CONCRETE_RTT (VNetworkHandle, Vca::VRolePlayer);

//  Aliases
public:
    typedef V::VAggregatePointer<VSpaceHandle> space_vector_t[M_POP_MaxObjectSpace + 1];

//  Construction
public:
    static ThisClass *New (char const *pPathName, bool bReadOnly);
private:
    VNetworkHandle (char const *pPathName, bool bReadOnly);
...
public:
    ...
    bool IsValid () const {
	return IsntNil (m_iMapping.RegionAddress ());
    }
...
class VSpaceHandle : public Vca::VRolePlayer {
    DECLARE_CONCRETE_RTT (VSpaceHandle, Vca::VRolePlayer);

    friend class VNetworkHandle;

//  Aliases
public:
    typedef VkDynamicArrayOf<V::VAggregatePointer<VSegmentHandle> > segment_vector_t;
...
public:
    ...
    size_t SegmentModelSize () const {
	return (size_t)(m_xMaxModelSegment + 1 - m_xMinModelSegment);
    }

protected:
    size_t SegmentVectorSize () const {
	return (size_t)(m_xMaxVectorSegment + 1 - m_xMinVectorSegment);
    }
...
//  State
protected:
    VNetworkHandle::Reference const m_pNetworkHandle;
    ...
    segment_vector_t		m_pSegmentVector;
};

...
class VSegmentHandle : public Vca::VRolePlayer {
    DECLARE_CONCRETE_RTT (VSegmentHandle, Vca::VRolePlayer);
...
//  Mapping
protected:
    void Unmap (bool bUnlocking);

    void Sync ();

    bool Map (bool bReadOnly, bool bLocking);

    bool MapNew (size_t sSegment);
...
class VConversionAgent : public Vca::VServerApplication {
    DECLARE_CONCRETE_RTT (VConversionAgent, Vca::VServerApplication);

//  Construction
public:
    VConversionAgent (Context *pContext);
...
protected:
    ...
    VNetworkHandle::Reference	m_pNetworkHandle;
    VkMemory			m_iMapping;
};

...
PrivateVarDef unsigned int BaseVersionsToModel = 1;

...
static double OperationRate (size_t sTransfer) {
...
PrivateFnDef void MessageType_VDisplayApplicationMessage (
    ...
    fflush  (stdout);
    fprintf (
	stderr, "%s %s\n", MessageType_Flag (xMessageType), VMakeMessage (pFormat, rArguments)
    );
    fflush  (stderr);
}
...
PrivateFnDef void Block_Convert (rtBLOCK_BlockType *pThis, size_t Unused(sThis)) {
...
PrivateFnDef void Closure_Convert (rtCLOSURE_Closure *pThis, size_t Unused(sThis)) {
...
PrivateFnDef void Descriptor_Convert (rtDSC_Descriptor *pThis, size_t Unused(sThis)) {
...
PrivateFnDef void Link_Convert (rtLINK_Type *pThis, size_t Unused(sThis)) {
    size_t		cRRD;
    rtLINK_RRDType*	pRRD;
    union linkFields_t {
	struct forward_t {
...
PrivateFnDef void LStore_Convert (rtLSTORE_Type_LStore *pThis, size_t Unused(sThis)) {
    size_t cBPT, iBPT;
...
PrivateFnDef void PArray_Convert (M_POP *pThis, size_t sThis) {
    size_t cPOP, iPOP;
...
PrivateFnDef void PCT_Convert (PS_CT *pThis, size_t Unused(sThis)) {
    size_t	cCTE;
    PS_CTE*	pCTE;
    union cteFields_t {
	struct forward_t {
...
PrivateFnDef void PToken_Convert (rtPTOKEN_Type *pThis, size_t Unused(sThis)) {
    size_t			cSDsExamined;
    rtPTOKEN_SDType*		pSD;
...
PrivateFnDef void POPVector_Convert (POPVECTOR_PVType *	pThis, size_t Unused(sThis)) {
    size_t cPOP, iPOP;
    M_POP* pPOPArray;
...
PrivateFnDef void Vector_Convert (rtVECTOR_Type *pThis, size_t Unused(sThis)) {
    size_t		sArray;
...
PrivateFnDef M_CPreamble *Container_Convert (
    M_CPreamble* pThis, M_CPreamble const *pLimit, int *pChecksum
) {
...
PrivateFnDef bool Segment_Convert (
    PS_SH *pSegment, size_t sSegment, char const *pSegmentPathName
) {
...
bool VConversionAgent::ExplicitConverter () {
    bool bExplicitConversions = false;

    CommandCursor iCommandCursor (this);
    char const*	pSegmentPathName = iCommandCursor.nextToken ();
    while (pSegmentPathName) {
	bExplicitConversions = true;
	...
	bool bSegmentFlaggedForRemoval = !Segment_Convert (
	...
	pSegmentPathName = iCommandCursor.nextToken ();
    }

    return bExplicitConversions;
}

...
)
{
	...
	MessageType_DisplayApplicationMessage (
	    MessageType_Warning, "Invalid %s Offset %u", pStructureName, oStructure
	);
	return;
    }
...
DEFINE_CONCRETE_RTT (VSegmentHandle);
...
VSegmentHandle::VSegmentHandle (VSpaceHandle *pSpace, PS_SD *pSD, char const *pPathName)
...
void VSegmentHandle::Unmap (bool bUnlocking) {
    if (m_cMappingLocks > 0 && bUnlocking && 0 == --m_cMappingLocks)
	m_iMapping.Destroy ();
}
...
bool VSegmentHandle::Map (bool bReadOnly, bool bLocking) {
    ...
    if (m_iMapping.Map (m_pPathName, bReadOnly, g_xMappingMode)) {
...
bool VSegmentHandle::MapNew (size_t sSegment) {
    Unmap (false);

    if (m_cMappingLocks > 0 || !m_iMapping.MapNew (m_pPathName, sSegment, g_xMappingMode))
	return false;
...
void VSegmentHandle::ReceiveIncomingMessage (
	...
	rename (m_pPathName, m_pSavedPathName);
#if defined(__VMS)
...
DEFINE_CONCRETE_RTT (VSpaceHandle);
...
void VSpaceHandle::CleanupOSD () const {
	...
	size_t sFileName = strspn (pFileName, "0123456789");
#ifdef __VMS
...
VSegmentHandle *VSpaceHandle::DeclareSegment (PS_SD* pSD) {
    if (m_pSegmentVector.elementCount () == 0) {
	...
	m_pSegmentVector.Guarantee (SegmentVectorSize ());
    }
    else if (m_xMinVectorSegment > pSD->segment) {
	size_t sSegmentVectorDisplacement = m_xMinVectorSegment - pSD->segment;
	size_t sSegmentVector = (size_t)(m_xMaxVectorSegment - pSD->segment + 1);
	m_pSegmentVector.Guarantee (sSegmentVector);

	while (sSegmentVector-- > sSegmentVectorDisplacement)
	    m_pSegmentVector[sSegmentVector].setTo (m_pSegmentVector[sSegmentVector - sSegmentVectorDisplacement]);
	while (sSegmentVector-- > 0)
	    m_pSegmentVector[sSegmentVector].clear ();

	m_xMinVectorSegment = pSD->segment;
    }
    else if (m_xMaxVectorSegment < pSD->segment) {
	m_pSegmentVector.Guarantee((pSD->segment - m_xMinVectorSegment + 1));
    ...
    if (!pSegmentHandle) {
	pSegmentHandle = new VSegmentHandle (this, pSD, MakeMessage ("%s/%u", m_pPathName, pSD->segment));
	m_pSegmentVector[pSD->segment - m_xMinVectorSegment].setTo (pSegmentHandle);
    }
...
VSpaceHandle::VSpaceHandle (
    VNetworkHandle *pNetworkHandle, char const *pOSDPathName, unsigned int xSpace
) : m_pNetworkHandle (pNetworkHandle), m_xSpace (xSpace)
{
...
DEFINE_CONCRETE_RTT (VNetworkHandle);
...
VNetworkHandle *VNetworkHandle::New (char const *pPathName, bool bReadOnly) {
    VNetworkHandle *pInstance = new VNetworkHandle (pPathName, bReadOnly);
...
VNetworkHandle::VNetworkHandle (char const *pPathName, bool bReadOnly) {
/*****  Verify that the NDF can be opened, ...  *****/
    if (!m_iMapping.Map (pPathName, bReadOnly, g_xMappingMode)) {
	MessageType_DisplaySystemMessage (
...
void VNetworkHandle::CleanupOSD () const {
	...
	VSpaceHandle::Reference pSpaceHandle (m_pSpaceHandle[xSpace]);
	if (pSpaceHandle)
...
VSegmentHandle* VNetworkHandle::SegmentHandle (
    ...
    VSegmentHandle *pSegmentHandle = 0;

    VSpaceHandle *pSpaceHandle = m_pSpaceHandle[xSpace];
    if (pSpaceHandle) {
	pSegmentHandle = pSpaceHandle->SegmentHandle (xSegment);
...
/* OS directory must exist */
    if (FindOrCreateDirectory ()) {
	for (unsigned int xSpace = 0; xSpace <= M_POP_MaxObjectSpace; xSpace++) {
	/* space and space directory must exist */
	    VSpaceHandle *pSpaceHandle = m_pSpaceHandle[xSpace];
	    if (pSpaceHandle && pSpaceHandle->FindOrCreateDirectory ()) {
		for (unsigned int xSegment = pSpaceHandle->m_xMinModelSegment; xSegment <= pSpaceHandle->m_xMaxModelSegment; xSegment++) {
		/* segment must be declared and must exist or be retrievable */
		    VSegmentHandle *pSegmentHandle = SegmentHandle (xSpace, xSegment);
		    if (pSegmentHandle && pSegmentHandle->FindOrRetrieveSegment ());
		    else if (Verbose) MessageType_DisplayApplicationMessage (
...
/*****  ... determine the number of compactions to overlook, ...  *****/
    unsigned int cBaseVersionsToModel = BaseVersionsToModel;
...
DEFINE_CONCRETE_RTT (VConversionAgent);
...
VConversionAgent::VConversionAgent (Context *pContext)
    : BaseClass		(pContext)
    , m_pSlavePathName	(commandStringValue ("n"))
    , m_pMasterPathName	(commandStringValue ("N"))
{
    Verbose = commandSwitchValue ("v");
    BaseVersionsToModel = atoi (commandStringValue ("b", 0, "0"));
}
...
VSegmentHandle* VConversionAgent::SegmentHandle (unsigned int xSpace, unsigned int xSegment) const {
    return m_pNetworkHandle ? m_pNetworkHandle->SegmentHandle (xSpace, xSegment) : 0;
}
...
bool VConversionAgent::MapNew (size_t sMapping) {
    Unmap ();
    return m_iMapping.MapNew (m_pSlavePathName, sMapping, g_xMappingMode);
}

...
bool VConversionAgent::start_() {
    if (!BaseClass::start_())
	return false;

    if (ExplicitConverter ()) {
    }
    else if (!m_pSlavePathName) {
    //  This is a server ...
	...
	freopen (commandStringValue ("s", 0, "/dev/null"), "a", stderr);
    }
    else if (m_pMasterPathName) {
    //  This is a client ...
	// ConnectPeer ();
    }
    else {
    //  This is a local maintainer...
	ProcessSlave ();
    }
...
void VConversionAgent::ProcessSlave () {
    m_pNetworkHandle.setTo (VNetworkHandle::New (m_pSlavePathName, false));
    if (m_pNetworkHandle) {
	m_pNetworkHandle->FindOrRetrieveSegments ();
	if (!commandSwitchValue ("p"))
...
int main (int argc, char* argv[]) {
    Vca::VOneAppMain_<VConversionAgent> iMain (argc, argv);
